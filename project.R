...
# Dataset only concerning regions variables (filtering by country code)
world1 <- vis.data %>% filter(`Country Code` == c("EAS","LCN","SAS", "ECS", "MEA", "NAC", "SSF")) 
...
# Static plot
ggplot(world1, aes(x = Year, y = SP.ADO.TFRT, colour =`Country Code`))+
  geom_point(alpha = 0.7) +
  labs(title = 'Adolescent fertility rate (births per 1,000 women ages 15-19)',
       subtitle = 'Year: {frame_time}',
       y = 'births per 1,000 women ages 15-19')  

# Animation plot
a<-ggplot(world1, aes(x = Year, y = SP.ADO.TFRT, colour =`Country Code`))+
  geom_point(alpha = 0.7) +
  ...
  shadow_mark(size = 1, colour = 'gray', past = T)

animate(a, duration = 15, fps = 10, renderer = gifski_renderer())
anim_save("moje_test.gif")